# 📊 GitHub Trending 日报 — 2026-08-11 (周二)

> 每日自动更新 · 数据来源: [GitHub](https://github.com/trending)

---

### 1. [KKKKhazix/human-writing](https://github.com/KKKKhazix/human-writing)

> 让 AI 写的中文读起来像一个具体的人在说话。通用创作与改稿 Skill，开箱即用。


🔤 **Python** · ⭐ **2,276** · 🍴 **199**

🏷️ `agent-skills` `chinese-writing` `creative-writing` `writing-skill`


### 2. [Binaryify/open-kimi-ppt-skill](https://github.com/Binaryify/open-kimi-ppt-skill)

> 非官方 Kimi Slides Skill：让 AI Agent 生成可编辑 PPTD + PPTX，并附带本地浏览器编辑器 Unofficial Kimi Slides skill for AI agents — generate editable PPTD + PPTX with a local browser editor


⭐ **1,607** · 🍴 **1,182**


### 3. [ShawnPana/phone-harness](https://github.com/ShawnPana/phone-harness)

> let your agent control your phone


🔤 **Python** · ⭐ **1,321** · 🍴 **103**


### 4. [oil-oil/oil-motion](https://github.com/oil-oil/oil-motion)

> Create smooth, responsive interactive web animations.


🔤 **Python** · ⭐ **1,203** · 🍴 **102**


### 5. [MengTo/kage](https://github.com/MengTo/kage)

> An interactive five-chapter night walk through a Kyoto mountain temple, rendered live in Three.js.


🔤 **HTML** · ⭐ **774** · 🍴 **136**

🏷️ `creative-coding` `generative-art` `interactive-storytelling` `japanese-design` `threejs` `webgl`


### 6. [SMNETSTUDIO/WeChat-AI](https://github.com/SMNETSTUDIO/WeChat-AI)


🔤 **TypeScript** · ⭐ **748** · 🍴 **555**


### 7. [xoreaxeaxeax/asm-hall-of-shame](https://github.com/xoreaxeaxeax/asm-hall-of-shame)

> Racing to the bottom of CPU performance


🔤 **C** · ⭐ **730** · 🍴 **10**


### 8. [eternityspring/shuohao-skills](https://github.com/eternityspring/shuohao-skills)

> AI 短剧制作的 skill 集合：拆角色、出设定图、排大纲 | Agent skills for AI short-drama production — character bibles, model sheets, adaptation outlines. Runs in Claude Code & codex.


🔤 **JavaScript** · ⭐ **648** · 🍴 **77**


### 9. [T8mars/comfyui-minimax-h3-audio-T8](https://github.com/T8mars/comfyui-minimax-h3-audio-T8)


🔤 **Python** · ⭐ **586** · 🍴 **30**


### 10. [sv-number/mcp-server](https://github.com/sv-number/mcp-server)

> MCP server for AI agents that need a phone number: order a private number in 200+ countries, read the SMS verification code, hand it back. The widest country coverage in the category, and you can check it with one API call.


🔤 **JavaScript** · ⭐ **549** · 🍴 **0**

🏷️ `ai-agents` `claude` `claude-code` `cline` `cursor` `mcp`


### 11. [Appllama/top-welcome-screens](https://github.com/Appllama/top-welcome-screens)

> How top iOS apps say hello | 10 animated welcome screens inspired by top-earning apps. Rebuilt in React Native + Expo.


🔤 **TypeScript** · ⭐ **420** · 🍴 **39**

🏷️ `design` `design-patterns` `expo` `ios` `ios-app` `motion`


### 12. [Larryvrh/ComfyUI-MiniMax-H3-Turbo](https://github.com/Larryvrh/ComfyUI-MiniMax-H3-Turbo)


🔤 **Python** · ⭐ **372** · 🍴 **24**


### 13. [mewamew/my_ai_town](https://github.com/mewamew/my_ai_town)

> 我的AI小镇


🔤 **GDScript** · ⭐ **364** · 🍴 **67**


### 14. [Sateezg/codex-bridge](https://github.com/Sateezg/codex-bridge)

> Image generation (gpt-image-2) and GPT-5 subagents for Claude Code — through the Codex CLI login you already have. No OpenAI API key.


🔤 **Shell** · ⭐ **357** · 🍴 **3**

🏷️ `ai-agents` `claude-code` `claude-code-plugin` `codex` `gpt-5` `gpt-image-2`


### 15. [sowarma/wp2shell-PoC](https://github.com/sowarma/wp2shell-PoC)

> CVE-2026-63030 & CVE-2026-60137 RCE chain proof-of-concept


🔤 **Python** · ⭐ **351** · 🍴 **22**

🏷️ `cve` `cve-2026-60137` `cve-2026-63030` `data-analysis` `wp2shell` `wp2shell-poc`


### 16. [jmarshall23/Quake_4_Alpha](https://github.com/jmarshall23/Quake_4_Alpha)

> Quake 4 alpha released for preservation purposes only. 


🔤 **C++** · ⭐ **329** · 🍴 **43**


### 17. [NikoDemon80/ComfyUI-H3-Motion-Context](https://github.com/NikoDemon80/ComfyUI-H3-Motion-Context)

> Clip chaining for MiniMax H3 in ComfyUI - motion and audio genuinely continue across joins


🔤 **Python** · ⭐ **327** · 🍴 **23**

🏷️ `comfyui` `comfyui-nodes` `minimax` `video-generation`


### 18. [tanishqkancharla/calldiff](https://github.com/tanishqkancharla/calldiff)

> Diffs for function call stacks across git commits. 22 languages supported (AST-based, built using Tree-sitter). Great for agentic review.


🔤 **TypeScript** · ⭐ **298** · 🍴 **14**


### 19. [pengchujin/tower](https://github.com/pengchujin/tower)

> 塔台 — 原生 SwiftUI iOS App，在本机管理机场订阅与自有节点，生成 Surge / Clash / Shadowrocket / Loon / Quantumult X 配置


🔤 **Swift** · ⭐ **285** · 🍴 **16**


### 20. [robonuggets/gauntlet-loop](https://github.com/robonuggets/gauntlet-loop)

> Turn any goal into a short prompt that makes your agent set a real quality bar, run builder and critic pairs, compare blind, and loop until it wins.


⭐ **265** · 🍴 **32**


---

## 🤝 支持这个项目

如果这个日报对你有帮助，欢迎赞助支持持续开发！


![支付宝赞赏码](assets/alipay_qr.jpg)


*最后更新: 2026-08-11 09:17:45*
